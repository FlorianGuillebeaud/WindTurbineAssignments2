...
global Theta_cone Theta_tilt Theta_yaw
Theta_pitch = 0.0 ; % [rad]
Theta_cone = 0 ; % [rad]
Theta_tilt = 0 ; % [rad]
Theta_yaw = 0 ; % [rad]
...
%         for k=1:N_element
%             % k
%             [Vrel_y, Vrel_z] = velocity_compute(b, blade_data(k), H, Ls, Wy(b,k,i-1), Wz(b,k,i-1), Theta_wing1(i), Theta_wing2(i), Theta_wing3(i),omega ) ;
...
%omega(1) = 0.673 ; % [rad/s] rotational speed
omega(1) = 0.5 ;
...
%% Initialization %%
clear Wy Wz a_rem p time Theta_wing1 Theta_wing2 Theta_wing3 py pz dm dP 
V0y = 0 ;
V0z = V_0 ;
Wy = zeros(B,N_element,N) ;
Wz =  zeros(B,N_element,N) ;
...
for i=2:N*2
    ...
    for b=1:B
        ...
        for k=1:N_element
            % k
            [Vrel_y, Vrel_z] = velocity_compute(b, blade_data(k), H, Ls, Wy(b,k,i-1), Wz(b,k,i-1), Theta_wing1(i), Theta_wing2(i), Theta_wing3(i), omega(i-1) ) ;
            %Vrel_y = -omega*blade_data(k)+Wy(b,k,i-1) ;  
            %Vrel_z = V_0+Wz(b,k,i-1);

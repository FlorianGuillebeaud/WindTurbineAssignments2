function [thrust, Power, Maero, omega, Theta_pitch, time, MG]=TURB_BEM_PIcontrol(H, Ls, R, B, omega0, V_0, rho, delta_t, N, N_element, Theta_pitch0, Theta_cone, Theta_tilt, Theta_yaw, Kk, Ki, Kp, Irotor)
...
%Initialization 
global W3_100 W3_60 W3_48 W3_36 W3_30 W3_24 blade_data M_G omega_list pz_TS
Theta_pitch_i(1)=0;
...
for i=2:N
    ...
    for b=1:3
        ...
        % power computation 
        Power_b(b,i) = trapz(blade_data(:,1), real(dP(:,i))) ;
        % torque
        Maero_b(b,i)=trapz(blade_data(:,1), real(dm(:,i))) ;
    end
    Thrust (i-1)= sum(thrust(:,i)) ;
    Power(i-1)=sum(Power_b(:,i));
    Maero(i-1)=sum(Maero_b(:,i));
